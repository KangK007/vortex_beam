"""Supplementary LG basis-waist sensitivity：LG 分析基腰 w_an 对归一化模态谱宽的影响。
...
SCRIPT = "lg_basis_waist_sensitivity"
...
def widths_for_basis(alpha: float, fork_ell: float, basis_waist: float, params, X, Y, R, PHI, dx, dy):
    ...
    # Supplement LG basis-waist sensitivity 讨论的是 LG-basis waist sensitivity，
...
def main() -> None:
    # 论文参数指示：LG basis-waist sensitivity 最终基腰敏感性建议设置为：
    ...
    fig.suptitle("LG basis-waist sensitivity  Basis-waist sensitivity")
    fig.savefig(out_dir / "lg_basis_waist_sensitivity.png", dpi=300)
    save_npz(out_dir, "lg_basis_waist_sensitivity", waist_ratios=waist_ratios, sigma_l_norm=sigma_l_norm, sigma_p_norm=sigma_p_norm)
    save_params(out_dir, "lg_basis_waist_sensitivity", {"params": params, "alpha": alpha, "cases": cases, "dx": dx, "dy": dy})
    plt.close(fig)
