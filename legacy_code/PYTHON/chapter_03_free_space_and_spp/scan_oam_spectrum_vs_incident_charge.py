"""OAM spectrum vs incident charge：固定 SPP 调制荷 q，改变入射分数阶拓扑荷 alpha 后的 FVVB OAM 谱。
...
SCRIPT = "oam_spectrum_vs_incident_charge"


def main() -> None:
    # ????????? SPP ??? alpha ????????
    # grid_n=400, rho_max_factor=8.0, n_min=-200, n_max=200, lmax=40,
    # nr_fourier=320, nphi_fourier=720?fork_ell ??? SPP ??????? q=0.5 ???
    ...
    fig.suptitle(rf"OAM spectrum vs incident charge  FVVB under fixed SPP ($q={fork_ell:.1f}$)")
    fig.savefig(out_dir / "oam_spectrum_vs_incident_charge.png", dpi=300)
    save_npz(out_dir, "oam_spectrum_vs_incident_charge", alpha_list=np.array(alpha_list), l_list=l_list, spectra=np.array(spectra))
    save_params(out_dir, "oam_spectrum_vs_incident_charge", {"params": params, "fork_ell": fork_ell, "alpha_list": alpha_list, "dx": dx, "dy": dy})
    plt.close(fig)
